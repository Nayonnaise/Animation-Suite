"""Animation Function: Plots the last 100 (n) points with fade around the latest point"""
...
def Animate(resX, resY, path, i, win ,radius, colour, background):
    ...
    if i < n:  # When len(path) is less than the value of n
        for k in range(i):
            ...
            # Draws the Point
            pygame.draw.circle(win, (r, g, b), [int((resX / 2) + (resY / 4 * path[k, 0]) - (resY / 4 * path[i, 0])),
                                        int((resY / 2) + (resY / 4 * path[k, 1]) - (resY / 4 * path[i, 1]))], radius)
    else:
        for k in range(n):
            ...
            pygame.draw.circle(win, (r, g, b), [int((resX / 2) + (resY / 4 * path[i-(n-k), 0]) - (resY / 4 * path[i, 0])),
                                        int((resY / 2) + (resY / 4 * path[i-(n-k), 1]) - (resY / 4 * path[i, 1]))], radius)
